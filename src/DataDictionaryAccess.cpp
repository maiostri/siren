...
string DataDictionaryAccess::RunSQL(string sql) {
...
string DataDictionaryAccess::RunSQL(Token *tokenlist) {
...
string DataDictionaryAccess::RunInsertFile(string sql, string filename) {
...
void DataDictionaryAccess::DropSchema(vector<string> *commandlist) {
	string sql;
	string insert;
	...
	while (!query->Eof()) {
		...
		try {
			...
			logUtils::getInstance().logError("Error on removing the index files: " + string(E.what()));
		}
...
void DataDictionaryAccess::CreateSchema(vector<string> *commandlist) {
	string sql;
	char filedir[200];
...
int DataDictionaryAccess::IsExtractor(string extractorname) {
...
bool DataDictionaryAccess::IsValidExtractor(string extractorname, Token *tokenlist) {
...
bool DataDictionaryAccess::IsExtractorParameter(int extractorcode, string parametername) {
	bool res = false;
	sql = "select count(ParameterName) as amount from EPD$Parameters where ExtractorCode = "
			+ lexical_cast<string>(extractorcode);
	sql += " and ParameterName = '" + parametername + "'";
	query->Open(sql);
	if (!query->Eof())
...
int DataDictionaryAccess::IsMetric(string metricname) {
	string temp;
	int ret = 0;
...
bool DataDictionaryAccess::IsParticulateMetric(string metricname) {
...
bool DataDictionaryAccess::IsStillimageAttribute(Token *tokenlist, string attribname) {
...
bool DataDictionaryAccess::IsParticulateAttribute(Token *tokenlist, string attribname) {
...
bool DataDictionaryAccess::IsAudioAttribute(Token *tokenlist, string attribname) {
...
string DataDictionaryAccess::IsComplexAttributeReference(string tablename, string attribname) {
...
bool DataDictionaryAccess::IsStillimageAttributeReference(string tablename, string imgattribname) {
	bool res = false;
	string sql;
...
bool DataDictionaryAccess::IsStillimageAttributeReference(string attribname) {
	bool res = false;
	string sql;
...
int DataDictionaryAccess::IsValidMetric(string metricname, string tablename, string attribname) {
	...
	if (result > 0) {
		...
		sql += "' and ComplexAttribName = '";
		sql += attribname;
		sql += "' and MetricCode = " + lexical_cast<string>(result);
		query->Open(sql);
		if (!query->Eof())
...
bool DataDictionaryAccess::IsComplexAttribIndex(string caindex) {
	bool result = false;
...
string DataDictionaryAccess::GetDefaultMetric(string tablename, string attribname) {
...
string DataDictionaryAccess::GetMetricType(int metriccode) {
	string result = "";

	sql = "select MetricType from CDD$MetricStruct where metriccode = " + lexical_cast<string>(metriccode);
	query->Open(sql);
	if (!query->Eof()) {
		result = query->StringFieldByIndex(0);
	}
...
int DataDictionaryAccess::GetMetricCode(string metricname) {
...
void DataDictionaryAccess::GetMetrics(string tablename, string attribname, vector<string> *complexattriblist,
...
string DataDictionaryAccess::GetIndexFile(string tablename, string attribname, string metricname) {
...
bool DataDictionaryAccess::verifyComplexFieldStatement(Token *tokenlist, const string& attribname) {
	...
	while ((tk2 != NULL) && (tk2->Lexem.compare("where") != 0)) {
		...
		if ((IsComplexAttributeReference(tname, attribname).compare("") != 0)) {
			return true;
		}
		if (tk2 != NULL)
			tk2 = tk2->Next;
...
string DataDictionaryAccess::getTableNameFromTokenList(Token *tokenlist, string attribname) {
	// auxiliary pointer to the token "from"
	Token *p_from = tokenlist;
	while (p_from->Lexem.compare("from") != 0) {
		p_from = p_from->Next;
...
int DataDictionaryAccess::GetDistanceFunction(string tablename, string attribname, string metricname) {
...
void DataDictionaryAccess::GetParameters(int metriccode, vector<string> *attriblist) {
...
void DataDictionaryAccess::DropComplexIndex(string indexname) {
...
void DataDictionaryAccess::UpdateTableComplexAttribMetric(string tablename, string imageattr) {
...
//---------------------------------------------------------------------------
void DataDictionaryAccess::CommitTransaction() {
	SqlConnection->Commit();
}

//---------------------------------------------------------------------------

void DataDictionaryAccess::RollbackTransaction() {
}

//---------------------------------------------------------------------------

void DataDictionaryAccess::CreateTableStillimage(Token *tokenlist, vector<string> *createlist) {
	//ORACLE
	...
	string command;
	if (getSirenQueries()->getTypeConnection() == POSTGRESQL) {
		command = "create table IPV$" + tablename + "_" + columnname
				+ " ( image_id integer primary key, image OID, foreign key (image_id) references " + tablename + " ("
				+ columnname + "))";
	} else if (getSirenQueries()->getTypeConnection() == ORACLE) {
		command = "create table IPV$" + tablename + "_" + columnname
...
void DataDictionaryAccess::CreateTableAudio(Token *tokenlist, vector<string> *createlist) {
	//ORACLE
...
void DataDictionaryAccess::CreateTableInsertComplexAttribute(vector<string> *commandlist, Token *tokenlist,
...
void DataDictionaryAccess::CreateTableInsertComplexAttribMetric(Token *tokenlist, string tablename,
	...
	treename += lexical_cast<string>(indexcode);
	treename += ".dat";
	...
	if (tokenlist->Next->Lexem.compare("table") == 0) {
		...
		indexname = "index";
		indexname += lexical_cast<string>(indexcode);
	} else {
	...
	command = command + tablename + "', '" + complexattribname + "', " + lexical_cast<string>(metriccode) + ", '"
			+ IsDefault + "', '" + indexname + "', '" + treename + "')";
	commandlist->push_back(command);
}

//---------------------------------------------------------------------------

void DataDictionaryAccess::CreateTableAlterPVTable(string attribtype, string tablename, vector<string> *commandlist,
		string complexattribname, int metriccode, vector<string> *complexattriblist, vector<string> *metricreflist) {
...
bool DataDictionaryAccess::SelectAsteriskStatement(Token *tokenlist, int scope) {
...
string DataDictionaryAccess::SelectColumnRefListStatement(Token *tokenlist, int scope) {
...
void DataDictionaryAccess::AddMMColRef(Token **tk1, Token **tk1_p, Token **tk2, string mmtabletype, string tablename,
...
void DataDictionaryAccess::NearAndFarPredicates(Token **tokenlist, int activetokenlist) {
	...
	// query objects
	vector<tDynamicObject *> *queryobjects = NULL;
	...
	while (list != NULL) {
		...
		if ((list->Lexem.compare("near") != 0) && (list->Lexem.compare("far") != 0)) {
			...
			if (list->Prior->Prior->Lexem.compare(".") == 0) {
				...
				tablename = getTableNameFromTokenList(tokenlist[activetokenlist], attribname);
			}
			...
			if (list->TokenType == TK_OPEN_BRACE) {
				...
				if (list->Lexem.compare("subselect") == 0) {
					...
					// recursive call
					NearAndFarPredicates(tokenlist, list->ScopePosition);
					...
					if (this->getSirenQueries()->getTypeConnection() == POSTGRESQL) {
						sql = "select count(*) as amount from (" + subquery + ") as subquery";
					} else if (this->getSirenQueries()->getTypeConnection() == ORACLE) {
					...
					if (!query->Eof()) {
						...
						if (auxcount == 0) {
							throw QueryReturnedZeroRecordsException(subquery);
						}
					}
					...
					if (attribtype.compare("particulate") == 0) {
						...
						if ((sgtype.compare("") == 0) && (auxcount != 1)) {
							...
							while (!query->Eof()) {
								for (int i = 0; i < parametercount; i++) {
									...
									paramtype = getSirenQueries()->getParticleType(tablename, attrlist->at(i));
									if (paramtype.compare("") == 0) {
										throw InvalidParameterReferenceException(parameterlist->at(i));
									} else {
										parametertypelist->push_back(paramtype);
									}
									parametervaluelist->push_back(lexical_cast<string>(query->DoubleFieldByIndex(i)));
								}
							}
						}
				...
				} else // particulate_reference
				{
					...
					while (list->TokenType != TK_CLOSE_BRACE) {
						...
						if (list->TokenType == TK_INTEGER) {
							parametertypelist->push_back(this->sirenQueries->getDbmsDataType("integer"));
						} else if (list->TokenType == TK_FLOATING_POINT) {
							parametertypelist->push_back(this->sirenQueries->getDbmsDataType("float"));
						}
				...
				// skipping token 'subselect' / or token ')'
				list = list->Next;
			} // treats file name
			...
			else if ((list->TokenType == TK_IDENTIFIER) && (list->Lexem.compare("stop") != 0)) {
				...
				if ((list->Next != NULL) && (list->Next->Lexem.compare(".") == 0)) {
					...
					attribname2 = list->Lexem;
					tablename2 = getTableNameFromTokenList(tokenlist[activetokenlist], attribname2);
				}
				list = list->Next;
			}
			...
			if (list != NULL) {
				if (list->Lexem.compare("by") == 0) {
					...
					if ((attribtype.compare("stillimage") == 0) || (attribtype.compare("audio") == 0)) {
						...
						if (metriccode <= 0) {
							throw InvalidMetricException(metricname);
						}
					} else // particulate
					{
						int metriccode = IsValidMetric(metricname, tablename, attribname);
						if (metriccode > 0) {
							...
							if ((paramauxlist->size()) / 2 == parameterlist->size()) {
								for (unsigned int j = 0; j < paramauxlist->size(); j += 2) {
									...
									int ix = -1;
									int param = lexical_cast<int>(paramauxlist->at(j).c_str());
									if (parameterlist->size() != 0 && (param <= parameterlist->size())) {
										ix = lexical_cast<int>(parameterlist->at(param).c_str());
									}
									if (ix == -1) {
										throw MissingParameterException(paramauxlist->at(j));
									} else {
										if (((paramauxlist->at(j + 1)).compare(parametertypelist->at(ix).c_str()))
												!= 0) {
											throw InvalidParameterTypeException(paramauxlist->at(j + 1),
													parametertypelist->at(ix));
										}
									}
								}
							} else {
				...
				if (attribname2.compare("") == 0) {
					...
					ProcessNearAndFarPredicates(&activelisttail, filequeryobject, predicate, tablename, attribname,
							sgtype, metricname, imagefilename, knn, range, tielist, queryobjects, queryobjectscount);
				} else {
					ProcessJoinQueries(&activelisttail, activetokenlist, predicate, tablename, tablename2, attribname,
							attribname2, sgtype, metricname, knn, range, tielist, attribtype, parameterlist,
							parametervaluelist, parametertypelist);
				}
				//Clean parameterlist
				...
				throw MissingStopAfterOrRangeException();
			}
		}
...
DataDictionaryAccess::BuildSelectionObjects(string tablename, string attribname, string metricname, string filename,
		string subselect, string attribtype, vector<string>* parameterlist, vector<string> parametervaluelist,
		int parametercount, int &returncount) {
...
// if the user didn't specified the metric name, get the default
	if (metricname.compare("") == 0) {
		...
		if (metricname.compare("") == 0) {
			throw NoDefaultMetricDefinedException(tablename, attribname);
		}
	}
	...
	if (attribtype.compare("stillimage") == 0) {
		...
		if (filename.compare("") != 0) {
			logUtils::getInstance().logInfo("Build Selection Objects - Extract based on a file name.");
			tDynamicObject *tObject = new tDynamicObject();
			Image *img = imageUtils::getInstance().createImageObjectByFilename(filename);
			logUtils::getInstance().logInfo("Get the extractor name.");
		...
		// From sub select
			...
			logUtils::getInstance().logInfo("Build Selection Objects - Extract based on a sub select.");
			// And here we go.
		...
		if (neededattribute.compare("") == 0) {
			throw MissingParticleForParticulateAttributeException(neededattribute, metricname);
		}
		...
		for (int i = 0; i < numberofobjects; i++) {
			tDynamicObject *tObject = new tDynamicObject();
			// vector constructor magic. :}
			const vector<string> subList(parametervaluelist.begin() + (i * parametercount),
					parametervaluelist.begin() + ((i + 1) * parametercount));
...
void DataDictionaryAccess::ProcessNearAndFarPredicates(Token **activelisttail, Token *filequeryobject, string predicate,
		string tablename, string attribname, string sgtype, string metricname, string imagefilename, string knn,
		string range, bool tielist, vector<tDynamicObject*>* obj, int numberofobjects) {
...
// setting the weights
	...
	if (predicate.compare("near") != 0) {
		throw InvalidPredicateException(predicate);
	}
	time_t tmp1 = time(NULL);
	if (sgtype.compare("") != 0) {
		// Converting to the dummy/slim tree array.
		tDynamicObject** object = new tDynamicObject*[obj->size()];
		copy(obj->begin(), obj->end(), object);
		if (knn.compare("") != 0) {
			int iknn = lexical_cast<int>(knn.c_str());
			if (sgtype.compare("sum") == 0)
				result = dummytree->SumNearestQuery(object, numberofobjects, iknn, tielist);
			else if (sgtype.compare("all") == 0)
				result = dummytree->AllNearestQuery(object, numberofobjects, iknn, tielist);
			else if (sgtype.compare("max") == 0)
				result = dummytree->MaxNearestQuery(object, numberofobjects, iknn, tielist);
			else {
				throw InvalidSimilarityGroupingTypeException(sgtype);
			}
		} else {
			if (range.compare("") == 0)
				range = "0";
			...
			if (sgtype.compare("sum") == 0)
				result = dummytree->SumRangeQuery(object, numberofobjects, temp);
			else if (sgtype.compare("all") == 0)
				result = dummytree->AllRangeQuery(object, numberofobjects, temp);
			else if (sgtype.compare("max") == 0)
				result = dummytree->MaxRangeQuery(object, numberofobjects, temp);
			else {
				throw InvalidSimilarityGroupingTypeException(sgtype);
			}
		}
	...
	else if (knn.compare("") != 0) {
		result = slimtree->NearestQuery(obj->at(0), lexical_cast<int>(knn.c_str()), tielist);
	}
	// range select query
	else if (range.compare("") != 0) {
		double temp = lexical_cast<float>(range.c_str());
		result = slimtree->RangeQuery(obj->at(0), temp);
	}        // if no query type informed, do a Range 0
	else {
		result = slimtree->RangeQuery(obj->at(0), 0.0);
		logUtils::getInstance().logInfo("No query type informed, doing a Range 0.");
	}
	time_t tmp2 = time(NULL);
	MetricTreeTotalTime += tmp2 - tmp1;

// Edit the statement to mount the 'in'
...
string DataDictionaryAccess::ProcessJoinQueries(Token **activelisttail, int activetokenlist, string predicate,
		string tablename, string tablename2, string attribname, string attribname2, string sgtype, string metricname,
		string knn, string range, bool tielist, string attribtype, vector<string> * parameterlist,
...
tDynamicObject *
DataDictionaryAccess::BuildParticulateObject(const int& partid, const vector<string>& UserParamList,
	...
	for (unsigned int i = 0; i < MetricParamList.size(); i++) {
		...
		int idx = sirenUtils::getInstance().getPositionCopy(UserParamList, MetricParamList.at(i));
		if (idx == -1) {
// if a particle was not found, the code should not have reached here, well, let's treat it anyway!
			logUtils::getInstance().logError("Particle not found.");
			return NULL;
		}

		// adding the value of the parameter being passed in its respective position on the metric
		if (parametertype.compare("number") == 0 || parametertype.compare("float") == 0
				|| parametertype.compare("numeric") == 0 || parametertype.compare("real") == 0) {

			double value = lexical_cast<double>(UserParamValueList.at(idx));
...
string DataDictionaryAccess::CheckInsertParticles(string tablename, vector<string> *InsertColList) {
	string result = "";
...
void DataDictionaryAccess::InsertParticulate(const string& tablename, const int& partid,
...
void DataDictionaryAccess::IndexParticulateAttribute(const string& tablename, const int& partid,
		const string& particulatename, const vector<string> &InsertColList, const vector<string>& InsertValueList) {
...
// for each metric associated with this table attribute
	for (unsigned int i = 0; i < MetricName->size(); i++) {
		...
		vector<vector<string> *> *particulateData = sirenQueries->getParticulateData(tablename, MetricName->at(i),
				particulatename);

		vector<string> *MetricParamNames = particulateData->at(0);
		vector<string> *MetricParamTypes = particulateData->at(2);
		vector<string> *parameterInstanceVector = new vector<string>();
		vector<string> *UserParamList = getSirenQueries()->convertAttributesToMetricParams(tablename, MetricName->at(i),
				particulatename, InsertColList, *parameterInstanceVector);

		vector<string> *inputValuesParams = new vector<string>();

		for (string param : *parameterInstanceVector) {
			int idx = sirenUtils::getInstance().getPositionCopy(InsertColList, param);
			if (idx != -1) {
				inputValuesParams->push_back(InsertValueList.at(idx));
			}
		}
		...
		tDynamicObject *obj = BuildParticulateObject(partid, *UserParamList, *inputValuesParams, *MetricParamNames,
				*MetricParamTypes);

		slimtree->Add(obj);
		dummytree->Add(obj);
		logUtils::getInstance().logInfo("Inserted object in to the slim and dummy tree.");
		delete particulateData;
		delete UserParamList;
		delete parameterInstanceVector;
	}
...
// The first position must be complex attribute itself.
...
// That's it. I DO NOT FUCKING NEED TO UPDATE in another loop! Motherfucker performance!
	...
	while (!query->Eof()) {
		...
		logUtils::getInstance().logInfo("Get the distance function.");
		...
		// inserting in the metric tree
		logUtils::getInstance().logInfo("Starting extracting object.");
		tDynamicObject *obj = buildObjectFromImage(tablename, stillimagename, metricName, imagefile, imageid, extractor,
				parameter);
		slimtree->Add(obj);
		logUtils::getInstance().logInfo("tDynamicObject added to the slim tree.");
		dummytree->Add(obj);
		logUtils::getInstance().logInfo("tDynamicObject added to the dummy tree.");
		...
		fieldsVector->push_back(parameter);

	}

// Here. Insert the image in the database here.
// For oracle, I need to mount the insert statement, execute it, search and return the blobs to populate them.
// Mounts the insert.
	string tableNameInsert = "ipv$" + tablename + "_" + stillimagename;
	query->mountInsert(tableNameInsert, *fieldsVector, *tDynamicCharVector, imageid, imagefile);
...
tDynamicObject *
DataDictionaryAccess::buildObjectFromImage(string& tablename, string& complexattribname, string& metricname,
		string& filename, long& oid, string& extractorName, string &parameterName) {
	Image* img = imageUtils::getInstance().createImageObjectByFilename(filename);
	logUtils::getInstance().logInfo("Create the Artemis object based on " + filename);

// Get the extractor names and parameters
// @TODO How about getting/setting the weights here?
	logUtils::getInstance().logInfo("Get the extractor(s) on database");
// @TODO For a complex attribute, if there were more than one extractor for that metric,
// this won't work.
// I need to return a vector of dynamicObjects.
...
// Instantiate the new object
	...
	while (!query->Eof()) {
		...
		extractorName = query->StringFieldByIndex(0);
		parameterName = query->StringFieldByIndex(1);
		// extract the characteristics
		logUtils::getInstance().logInfo(
				"Extract the parameter " + parameterName + " with the extractor: " + extractorName);
		obj->Extract(extractorName, parameterName, img);
...
void DataDictionaryAccess::SetWeights(tDynamicDistanceEvaluator *evaluator, string tablename, string attribname,
		string metricname) {
...
string DataDictionaryAccess::GetClientFolder() {
...
SirenResult * DataDictionaryAccess::runSelect(const string& sql) {

	this->getSirenQueries()->getQuery()->Open(sql);
	SirenResult *sirenResult = new SirenResult(*this->getSirenQueries()->getQuery());
	return sirenResult;
}
